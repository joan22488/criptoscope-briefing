...
// ── Portada editorial gpt-image-1 (CriptoScope Visual Style v4: sala de trading institucional) ──
// CRITICAL RULE reforzada al inicio Y al final: nunca cifras/precios/porcentajes legibles con precisión
// (serían datos inventados por la IA, no verificados — riesgo de que se confundan con datos reales).
const MASTER_PROMPT = `Ultra-realistic cinematic photograph for a premium institutional financial news outlet, in the visual tradition of a Bloomberg terminal room caught at 2am. CRITICAL RULE, NEVER VIOLATE: any on-screen chart, graph or dashboard panel must show ONLY abstract shapes, lines, bars or candlesticks with NO crisp readable numbers, prices, dollar amounts, percentages or tickers with specific values anywhere in the image — these would be fabricated data, not real, so they must stay visually abstract and blurred/illegible at the digit level even though the overall chart shape is clear. Scene: a dim, glass-walled institutional trading room or private office at night, one to three business-suited figures shown from behind, in profile, or as three-quarter silhouettes (never a clear frontal face), standing or seated, illuminated only by the cold glow of massive wall-mounted monitors. The dominant hero element is ONE oversized dashboard screen styled as a live financial monitoring interface relevant to the day's story — if the topic names a real company, exchange or platform (BlackRock, Coinbase, Strategy, an ETF issuer, a specific protocol), its name or wordmark can appear as on-screen text/label, rendered as a plausible approximation, alongside short institutional status labels like "LARGE MOVE DETECTED" or "CONFIRMED" or "LIVE MARKET DATA" — but never alongside a specific readable dollar figure or percentage. Smaller secondary monitors around the desk show the same abstract chart language. Beyond the glass, a dense night skyline glows in the distance, softly out of focus. Color palette: near-black navy and charcoal, cool blue-cyan screen glow, with emerald green (#00C896) reserved for the single most bullish/positive element on screen and a muted desaturated red for the bearish counterpart — consistent brand accent, used sparingly. Cinematic single-source lighting from the screens, dramatic long shadows, shallow depth of field, subtle film grain, premium editorial color grading. STRICTLY FORBIDDEN: memes, rockets, moons, bulls or bears as literal animals, laser eyes, cartoonish UI, visible keyboards or mouse in close-up, watermarks, any recognizable real person's face, clickbait-style oversized emoji graphics. 16:9. Reminder of the critical rule: every number on every screen must remain an abstract shape, never a legible specific figure.`;
